...
class CreateBACS:
    ...
    def __init__(self, file=None, test=True):
        ...
        try:
            self.df = pd.read_excel(source_file, dtype="object")
        ...
        if not self.df.empty:
            ...
            self.connected_to = self.get_connected_to()
    ...
    def get_ports(self):
        for row in range(8, len(self.df)):
            ...
            frame = self.get_frame(self.equipment.name)
...
#     [RangeIndex(start=0, stop=656, step=1), Index(['Unnamed: 0', 'Unnamed: 1', 'Unnamed: 2', 'Unnamed: 3', 'Unnamed: 4',
